...
public interface SvodResourceOfferorRepository extends JpaRepository<SvodResursOfferor, Long> {
    @Query(nativeQuery = true, value = "select * from svod_resurs_offeror where stroy_id = :stroy_id ;")
    List<SvodResursOfferor> findAllByStroy_Id(Integer stroy_id);

    boolean existsByUserIdAndStroy_Id(long userId, Integer stroy_id);

    @Query(nativeQuery = true, value = "select * from svod_resurs_offeror where stroy_id = :stroy_id and kodr = :kodr ;")
    Optional<SvodResursOfferor> findFirstByStroyAndKodr(Integer stroy_id, String kodr);

    boolean existsByStroy_Id(Integer stroy_id);
}
